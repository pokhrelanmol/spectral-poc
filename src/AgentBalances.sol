    ...
    UUPSUpgradeable
{
    ...
    modifier onlyAdminOrOwner() {
        require(
            ...
            "AgentBalances: caller is not the owner or admin"
        );
        _;
    }
    ...
    modifier onlyAgentTokenOrDeployer() {
        require(
            msg.sender == deployer ||
                IAutonomousAgentDeployer(deployer).isAgentToken(msg.sender),
            "AgentBalances: caller is not the deployer or admin"
        );
        _;
    }
    ...
    function initialize(address _admin) public initializer {
        require(
            _admin != address(0),
            "AgentBalances: admin cannot be the zero address"
        );
        __Ownable_init();
    ...
    function setDeployer(address _deployer) public onlyOwner {
        require(
            _deployer != address(0),
            "AgentBalances: deployer cannot be the zero address"
        );
        deployer = _deployer;
    }
    ...
    function setAgentWallet(
        ...
        require(
            ...
            "AgentBalances: agent wallet cannot be the zero address"
        );
        require(
            agent != address(0),
            "AgentBalances: agent cannot be the zero address"
        );
    ...
    function withdraw(address agent, address token, uint256 amount) public {
        require(
            ...
            "AgentBalances: caller is not the agent wallet"
        );
        require(
            agentBalance[agent][token] >= amount,
            "AgentBalances: insufficient balance"
        );
    ...
    function deposit(
        ...
        address agent,
        uint256 amount
    ) public onlyAgentTokenOrDeployer {
    ...
    function getLPBalance(
        ...
        address spectralToken = IAutonomousAgentDeployer(_deployer)
            .spectralToken();
        address lpToken = IUniswapV2Factory(
            IAutonomousAgentDeployer(_deployer).uniswapFactory()
        ).getPair(_token, spectralToken);
